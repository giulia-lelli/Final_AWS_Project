...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    with open(dogfile, "r") as infile:    # Reads in dognames from file, 1 name per line & automatically closes file
        ...
        while line != "":
            line = line.strip().lower()     # Process line by striping newline from line
            if line not in dognames_dic:
                dognames_dic[line] = 1     #check if the dogname(line) exists within dognames_dic, then if the dogname(line) doesn't exist within dognames_dic then add the dogname(line) to dognames_dic as the 'key' with the 'value' of 1. 
    ...
    for key in results_dic:
        ...
        if pet_label in dognames_dic:
            ...
            # appends (1,0) because only pet label is a dog. This indicates the pet label is-a-dog, classifier label is-NOT-a-dog.
            else:
